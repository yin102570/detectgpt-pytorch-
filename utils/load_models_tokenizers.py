...
def load_base_model_and_tokenizer(args, config, model_name=None):
    ...
    base_tokenizer = AutoTokenizer.from_pretrained(name, padding_side='left', **optional_tok_kwargs)
...
def load_mask_filling_model(args, config):
    ...
    mask_tokenizer = transformers.AutoTokenizer.from_pretrained(mask_filling_model_name,
                                                                model_max_length=n_positions, cache_dir=cache_dir)

    if args.dataset in ['english', 'german']:
        preproc_tokenizer = mask_tokenizer

    config["mask_model"] = mask_model
    config["mask_tokenizer"] = mask_tokenizer  # 修复：添加这行
